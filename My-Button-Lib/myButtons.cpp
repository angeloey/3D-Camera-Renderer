...
Button::Button(uint16_t xMin, uint16_t xMax, uint16_t yMin, uint16_t yMax, uint32_t colour, uint32_t symbolColour, uint8_t buttonType, TS_StateTypeDef& touchState):_xMin(xMin), _xMax(xMax), _yMin(yMin), _yMax(yMax), _colour(colour), _symbolColour(symbolColour), _buttonType(buttonType){
    ...
    up[1].X = xMax -5;                          up[1].Y = ((yMax - yMin)/2); 
    up[2].X = xMin +5;                          up[2].Y = ((yMax - yMin)/2);

        // Down facing arrow
    down[0].X = xMin + ((xMax - xMin)/2);       down[0].Y = yMax -5;
    down[1].X = xMax -5;                        down[1].Y = ((yMax - yMin)/2);
    down[2].X = xMin +5;                        down[2].Y = ((yMax - yMin)/2);

        // Left facing arrow
    left[0].X = xMin +5;                        left[0].Y = ((yMax - yMin)/2);
    left[1].X = ((xMax - xMin)/2);              left[1].Y = yMin +5; 
    left[2].X = ((xMax - xMin)/2);              left[2].Y = yMax -5; 

        // Right facing arrow
    right[0].X = xMax -5;                       right[0].Y = ((yMax - yMin)/2);
    right[1].X = ((xMax - xMin)/2);             right[1].Y = yMin +5;
    right[2].X = ((xMax - xMin)/2);             right[2].Y = yMax -5;
...
void Button::drawButton(){
    ...
    switch(_buttonType){
        case 0: //blank
            break;
        case 1: //up
            BSP_LCD_FillPolygon(up, 3);
            break;
        case 2: //down
            BSP_LCD_FillPolygon(down, 3);
            break;
        case 3: //left
        ...
        case 4: //right
            ...
            break;
        case 5: //topRight
            BSP_LCD_FillPolygon(topRight, 3);
            break;
        case 6: //bottomRight
            BSP_LCD_FillPolygon(bottomRight, 3);
            break;
        default:
            break;
    }
